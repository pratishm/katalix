...
`@lattix/cli` scaffolds Lattix projects without making runtime packages mandatory. The default command still creates a small Lattix Core project; passing a web router or native target opts into the app templates.
...
`--router` and `--target` are mutually exclusive. Generated apps use ecosystem tooling directly; Lattix provides manifests, renderers, and adapter contracts rather than replacing Vite, React Router, TanStack Router, Expo, or React Native.
