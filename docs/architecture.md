...
Lattix Core is built in **three explicit layers**. Each layer has a single responsibility; boundaries must not blur.

The optional Lattix App Runtime follows the same architecture shape in parallel: authoring DSL, normalized manifests, and adapters over existing ecosystem tools. Core remains independently usable without runtime packages.

## Lattix Core

### Layer A: UI Authoring DSL (`@lattix/dsl` — Phase 2 ✅)
...
### Layer B: Semantic UI Tree (`@lattix/core`)

The normalized semantic tree is the **source of truth**. Every fluent definition must resolve into `LattixNode` trees that are:
...
```ts
interface LattixNode {
  kind: string;
  id?: string;
  debugLabel?: string;
  props: Record<string, unknown>;
  style?: LattixStyle;
  animation?: LattixAnimation | LattixAnimation[];
  children?: LattixNode[];
...
`@lattix/core` ships base validators. `@lattix/diagnostics` (Phase 3 ✅) adds:
...
### Layer C: UI Renderer Adapters (`@lattix/react`, `@lattix/react-native` — Phase 5+)

Renderers consume **only** the normalized tree. They must not depend on partially-built chain state.

## Optional App Runtime

The App Runtime is a post-Core expansion. It should standardize app/runtime definitions without turning Lattix into a monolithic framework or custom runtime engine.
...
```
Authoring DSL  →  normalize  →  LattixNode tree  →  validate  →  renderer
```
...
- `@lattix/core`, `@lattix/dsl`, `@lattix/diagnostics`, `@lattix/tokens`
- Styles normalize to `normalizedStyle` with token/literal discrimination
- Built-in style diagnostics on every `toTree()`
- `@lattix/react` and `@lattix/react-native` render normalized semantic trees
- `@lattix/motion` defines shared motion presets, validation, and web/native adapters
- `@lattix/patterns` provides generic reusable composites built from primitives
- `@lattix/cli` scaffolds a minimal Lattix Core starter project plus optional Vite React, Expo, and plain React Native app templates
...
- `@lattix/app` defines optional app manifests for platform, provider, and environment declarations
- `@lattix/navigation` defines shared route manifests and adapter contracts for React Router, TanStack Router, and React Navigation
- `@lattix/data` defines API/server-state manifests and adapter contracts for fetch, TanStack Query, GraphQL, and RPC bindings
- `@lattix/storage` defines storage, database, and offline manifests with web/native adapter planning
- `@lattix/auth` defines auth/session manifests composing navigation, data, and storage refs
- `@lattix/web` defines browser metadata, PWA, rendering, route-boundary, browser capability, and security manifests
- `@lattix/native` defines native UX and capability manifests for Expo and plain React Native targets
- `@lattix/app` contains the bounded observability manifest area
